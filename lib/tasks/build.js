...
module.exports = function(config) {
	...
	if (!config.source.styles) {
		throw new api.errors.TaskError('No Stylus root specified');
	}
	...
	var watchConfig = {
		...
		styles: {
			files: [
				config.source.scripts + '/**/*.styl',
				config.source.styles + '/**/*.styl'
			],
			task: {
				task: stylus,
				config: stylusConfig
...
module.exports.defaults = {
	source: {
		...
		index: '<%= package.source.templates %>/index.html'
	},
	destination: {
		root: '<%= package.destination %>',
		index: '<%= package.destination %>/index.html',
		js: '<%= package.destination %>/js/app.js',
		css: '<%= package.destination %>/css/app.css'
	},
	debug: '<%= package.debug %>'
};
